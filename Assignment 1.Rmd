...
censusRace <- get_decennial(geography = "tract", 
  ...
  select(GOID, pct_RWhite, pct_RBlack, pct_RAI, pct_RAsian, pct_RNH, pct_ROther, pct_RTwoPlus)
...
acsEducation<- get_acs(geography = "tract", 
                   ...
                   output = "wide",
                   geometry = FALSE)%>%
  mutate(pct_EdHighSchool = EdHighschool / EdTotal)%>%
  mutate(pct_EdGED = EdGED / EdTotal)%>%
  mutate(pct_EdAssociates = EdAssociates / EdTotal)%>%
  mutate(pct_EdBachelors = EdBachelors / EdTotal)%>%
  mutate(pct_EdMasters = EdMasters / EdTotal)%>%
  mutate(pct_EdProfessional = EdProfessional / EdTotal)%>%
  mutate(pct_EdDoctorate = EdDoctorate / EdTotal)%>%
  select(GEID, pct_EdHighSchool, pct_EdGED, pct_EdAssociates, pct_EdBachelors, pct_EdMasters, pct_EdProfessional, pct_EdDoctorate)
